...
export default function YieldAnalysisChart({ data, unitName, }: YieldAnalysisChartProps) {
  // Calculate average yield
  const avgYield = data.reduce((sum, d) => sum + d.actualYield, 0) / data.length;
      ...
      return (
        <div className="bg-slate-800 border border-slate-600 rounded-lg p-3 shadow-xl">
          ...
          <div className="space-y-1">
            <p className="text-sm">
              <span className="text-blue-400 font-semibold">Actual Yield: </span>
    ...
    <div className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 rounded-xl p-6 border border-yellow-500/20">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-white">Yield Analysis - NBM Baseline</h2>
          </div>
          <p className="text-sm text-slate-400 mt-1">{unitName} - 24 Hour Performance</p>
        </div>
      ...
      <div className="flex items-start gap-2 bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 mb-4">
        ...
        <p className="text-xs text-slate-300">
          The <span className="text-yellow-400 font-semibold">shaded area</span> represents the Normal Behavior Model (NBM) baseline.
          The <span className="text-blue-400 font-semibold">blue line</span> shows actual yield.
          Deviations outside the baseline indicate anomalies requiring attention.
        </p>
      </div>
